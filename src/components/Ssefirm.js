...
const Ssefirm = () => {
    ...
    //Get all Sanghai Stock Enxhange companies for filtering
    ...
    */
    useEffect(() => {
       ...
       fetch(`https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=SHA:${id}&apikey=WF9H64N6MXWBW9AJ`)
       .then(response => {
           return response.json();
       })
    ...
    */
    ...
    const headerInfo = state.companies.filter(company => company.short === id).map(filteredCompany => (
        <h1 key={filteredCompany.short} className="mt-5 mb-4">
            {filteredCompany.name} Osake (SHA: {filteredCompany.short})
        </h1>
    ));   


    const info = state.companies.filter(company => company.short === id).map(filteredCompany => (
        <ul key={filteredCompany.short} className="list-group list-group-flush mt-5 firm-list">
